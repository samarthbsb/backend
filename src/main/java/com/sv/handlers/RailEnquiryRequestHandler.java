...
public class RailEnquiryRequestHandler implements IUrlRequestHandler
{
    ...
    @Override
    public HttpResponse handleRequest(String requestUri, String requestPayload, MessageEvent event, HttpRequest request)
        throws Exception
    {
        ...
        if (requestUri.matches("/v1/rail/liveStatus.*")) {
            ...
            return Utils.createResponse(gson.toJson(railApiService.getLiveStatus(trainno, stnfrom, date)), contentType,
            ...
            return Utils.createResponse(gson.toJson(railApiService.getTrainsAtStation(stnfrom, stnto, hr)),
            ...
            return Utils.createResponse(
                gson.toJson(railApiService.getSeatAvailabilty(stnfrom, stnto, date, cls, trainno, quota)), contentType,
                HttpResponseStatus.OK);
            ...
            String cls = Utils.getURLParam(urlParameters, "class");
            String date = Utils.getURLParam(urlParameters, "date");
            return Utils.createResponse(gson.toJson(railApiService.getTrains(stnfrom, stnto, date, cls)), contentType,
                HttpResponseStatus.OK);
            ...
            String date = Utils.getURLParam(urlParameters, "date");
            return Utils.createResponse(gson.toJson(railApiService.getFare(stnfrom, stnto, date, age, trainno, quota)),
                contentType, HttpResponseStatus.OK);
        } else if (requestUri.matches("/v1/rail/stations.*")) {
            String searchVal = Utils.getURLParam(urlParameters, "searchVal");
            return Utils.createResponse(gson.toJson(railApiService.getStations(searchVal)), contentType,
                HttpResponseStatus.OK);
        } else if (requestUri.matches("/v1/rail/route.*")) {
            String trainno = Utils.getURLParam(urlParameters, "trainno");
            return Utils.createResponse(gson.toJson(railApiService.getRoute(trainno)), contentType,
                HttpResponseStatus.OK);
        }else if (requestUri.matches("/v1/rail/pnrStatus.*")) {
            String pnr = Utils.getURLParam(urlParameters, "pnr");
            return Utils.createResponse(gson.toJson(railApiService.getPnrStatus(pnr)), contentType,
                HttpResponseStatus.OK);
        }
